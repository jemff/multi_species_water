...
print(fin_diff_mat_periodic(10))
...
#differentation_matrix(fidelity)
...
def outputs(R_max = 5, warmstart_info = None):
    ...
    for j in range(fidelity):
        ...
        beta_ff.append(2 * beta_i[1] * (upright_wc / (1 + upright_wc)) + beta_0*330/365*gamma*11**(-0.25))
        ...
        dHsig_z.append(-epsi[0] * state_l[j][-1] * c_z**2*beta_z[j] * rz/(c_z+state_l[j][-1] * sigma_z_l[j] * beta_z[j] * rz)**2 + state_l[j][1] * (
                    c_ff * sigma_ff_l[j] * beta_ff[j] * ff_satiation[j]) + gamma0/2*vel_z_l[j]**2 + p_z_l[j]*dz_vel_z_l[j])
        dHsig_ff.append(-(epsi[1] * state_l[j][0] * c_ff ** 2 * beta_ff[j] * sigma_z_l[j] * ff_satiation[j] ** 2) + gamma1/2*vel_ff_l[j]**2 + p_ff_l[j]*dz_vel_ff_l[j])
        ...
        dHv_z.append(gamma0*vel_z_l[j]*sigma_z_l[j]+dz_sigma_z_l[j]*p_z_l[j])
        dHv_ff.append(gamma1*vel_ff_l[j]*sigma_ff_l[j]+dz_sigma_ff_l[j]*p_ff_l[j])
        ...
        complementarity.append( dHv_z[j])
        complementarity.append( dHv_ff[j])
        compsum += complementarity[-1]**2
        compsum += complementarity[-2]**2
    ...
    trans_z = (D @ t_sigma_z + bc_mat @ (ca.hcat(dz_vel_z_l).T*ca.hcat(sigma_z_l).T + ca.hcat(dz_sigma_z_l).T*ca.hcat(vel_z_l).T))**2
    trans_ff = (D @ t_sigma_ff + bc_mat @ (ca.hcat(dz_vel_ff_l).T*ca.hcat(sigma_ff_l).T + ca.hcat(dz_sigma_ff_l).T*ca.hcat(vel_ff_l).T))**2
    ...
    f = pop_dyn_eqs + can_eqs + trans_eqs + inte @ compsum

    x = ca.vertcat(*[*vel_z_l, *vel_ff_l, *p_z_l, *p_ff_l, *sigma_z_l, *sigma_ff_l, *state_l])

    g = ca.vertcat(*[*prob_l])#, *complementarity])#, ca.reshape(canonical_p_z, (-1,1)), ca.reshape(canonical_sigma_z, (-1,1)), ca.reshape(canonical_p_ff, -1,1), ca.reshape(canonical_sigma_ff, (-1,1)), ca.reshape(canonical_sigma_ff, (-1,1)), ca.reshape(trans_ff, (-1,1))])
    lbg = np.zeros(g.size()[0])
    upper_zeros = 2*fidelity+2*fidelity*tot_points
    ubg = np.zeros(g.size()[0])#ca.vertcat(*[*np.zeros(upper_zeros)])#, (g.size()[0]-(upper_zeros))*[ca.inf]])
    ...
    if warmstart_info is None:
        ...
        s_opts = {'ipopt': {'print_level': 5, 'linear_solver': 'ma57', 'max_iter':2000}}#, 'hessian_approximation': 'limited-memory'}}  #
    else:
...
#            val[k] = outputs(R_max = res_vals[k])
    ...
    out = {'fidelity': fidelity, 'pts': tot_points, 'fineness':fineness, 'min': min, 'max': max, 'data':val}
