...
def output(tot_points = 20, fidelity = 20, Rmax = 5, Bmax = 0.1, warmstart_info = None, warmstart_opts = 1e-6, scalar = 'scalar2', hessian_approximation = True):
    ...
    for j in range(fidelity):
        ...
        state_l.append(np.array([0.16682589616059693, 0.0036831697755972926,11.27980207427809]).reshape(3,1))
    ...
    D_trans, D_diff_z = transport_matrix(depth=depth, total_points = tot_points, diffusivity = diffusivity_z)
    D_trans, D_diff_ff = transport_matrix(depth=depth, total_points = tot_points, diffusivity = diffusivity_ff)

    D_hjb = D_HJB(depth = depth, total_points=tot_points)

    J_z_p = i1 @ (M_per @ (t_Jsigma_z + D @ t_p_z + (D_hjb @ t_p_z.T).T * ca.hcat(vel_z_l).T - (D_diff_z @ t_p_z.T).T) ** 2 @ ones) /(tot_points-1) #((fidelity - 1) * (tot_points - 1)) #Repurposed to HJB
    J_z_v =  i1 @ (M_per @ (t_dJv_z + (D_hjb @ t_p_z.T).T)**2 @ ones)/(tot_points-1)#/((fidelity-1)*(tot_points-1))

    J_ff_p = i1 @ (M_per @ (t_Jsigma_ff + D @ t_p_ff + (D_hjb @ t_p_ff.T).T * ca.hcat(vel_ff_l).T - (D_diff_ff @ t_p_ff.T).T )**2 @ ones)/(tot_points-1) #/((fidelity-1)*(tot_points-1)) #Repurposed to HJB
    J_ff_v = i1 @ (M_per @ (t_dJv_ff + (D_hjb @ t_p_ff.T).T)**2 @ ones)/(tot_points-1) #((fidelity-1)*(tot_points-1))
    ...
    x = ca.vertcat(*[*vel_z_l, *vel_ff_l, *p_z_l, *p_ff_l, *sigma_z_l, *sigma_ff_l])

    g = ca.vertcat(*[*prob_l, trans_z_t, trans_ff_t,  J_z_v , J_ff_v, J_z_p, J_ff_p])#, ca.reshape(J_z_p, (-1,1)), ca.reshape(J_ff_p, (-1,1)), ca.reshape(J_ff_p, -1,1), ca.reshape(J_z_v, (-1,1)), ca.reshape(J_ff_v, (-1,1))])#, ca.reshape(trans_z,  (-1,1)), ca.reshape(trans_ff, (-1,1))])
    probs = 2*fidelity
    lbg = np.concatenate([np.zeros(probs+6)])#, np.repeat(-10**(-6), g.size()[0] - probs)])
    #upper_zeros = 2*fidelity+2*fidelity*tot_points
    ubg = np.concatenate([np.zeros(probs), 1e-8*np.ones(6)])#, np.repeat(10**(-6), g.size()[0] - probs)])
    #np.zeros(g.size()[0])#ca.vertcat(*[*np.zeros(upper_zeros)])#, (g.size()[0]-(upper_zeros))*[ca.inf]])
    lbx = ca.vertcat(fidelity*4*tot_points*[-ca.inf], np.zeros(tot_points*fidelity*2))
    ubx = ca.vertcat(*[[ca.inf]*(x.size()[0])])
    prob = {'x': x, 'f': f, 'g': g}
    ...
    if warmstart_info is None:
        ...
        s_opts = {'ipopt': {'print_level': 3, 'linear_solver': linsol, 'max_iter':50000, 'tol': 1e-8,
...
def increase_resolution(ir = 20, fr = 50, jumpsize = 5, Bmax = 0.1, Rmax = 5, warmstart_info = None, warmstart_opts=1e-3):
    ...
    for j in range(ir, fr+1, jumpsize):
        ...
        if counter is 0:
            ...
            carryover_trans = np.array([results[counter]['lam_g0'][-6:]]).squeeze()
        ...
        mult_dec_var = []
        for k in range(6):
            decision_vars.append(interp2d(x_vals, t_vals,  rs(results[counter]['x0'][k * j ** 2: (k + 1) * j ** 2], j), kind = 'cubic'))
            mult_dec_var.append(interp2d(x_vals, t_vals,  rs(results[counter]['lam_x0'][k * j ** 2: (k + 1) * j ** 2], j), kind = 'cubic'))
        mult_ineq = []
        for k in range(2):
    ...
    with open('data/' + 'hjb2_mfg_'+str(fr)+'_'+str(jumpsize) + "_R" + str(Rmax) + '.pkl', 'wb') as f:
        pkl.dump(results, f, pkl.HIGHEST_PROTOCOL)

    return results

increase_resolution(ir=10, fr=40, jumpsize=10,  Rmax=5)
