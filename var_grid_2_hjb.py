...
depth = 100
gamma0 = 1e-4  # 1e-9*(3600**2)
gamma1 = 1e-3  # 1e-8*(3600**2)


def output(tot_points = 5, fidelity = 20, Rmax = 5, Bmax = 0.1, warmstart_info = None, warmstart_opts = 1e-3, scalar = 'scalar2', hessian_approximation = True):
    ...
    ubg = np.concatenate([np.zeros(probs), 1e-8*np.ones(2), 1e-7*np.ones(5)])#, np.repeat(10**(-6), g.size()[0] - probs)])
    ...
    if warmstart_info is None:
        ...
        s_opts = {'ipopt': {'print_level': 3, 'linear_solver': linsol, 'max_iter':50000, 'tol': 1e-7,
...
def increase_resolution(ir_t = 20, fr_t = 50, ir_s = 20, fr_s = 50, jumpsize_t = 5, jumpsize_s = 5, save = True, Rmax = 5, warmstart_info = None, warmstart_opts = 10**(-6)):
    ...
    mult_ineq.append(results[0]['lam_g0'][-7:])
    #offset = 2*ir_t
    #for k in range(5):
    ...
    lam_g0_j.append(np.array([mult_ineq[-7:]]).squeeze())
    #for k in range(5):
    ...
    if save is True:
        with open('data/' + 'pdeco2'+str(fr_t*fr_s)+'_' + str(Rmax) + '.pkl', 'wb') as f:
...
def vary_resources(start = 5, stop = 50, steps = 45, ir_t = 10, fr_t = 70, ir_s = 10, fr_s = 70):
    ...
    grid_variation.append(increase_resolution(ir_t=ir_t, ir_s=ir_s, fr_t=fr_t, fr_s=fr_s, jumpsize_s=2, jumpsize_t=6, Rmax=resources[0]))
    print(resources)
    for i in range(1,steps):
        grid_variation.append(
            increase_resolution(ir_t=ir_t, ir_s=ir_s, fr_t=fr_t, fr_s=fr_s, jumpsize_s = 2, jumpsize_t = 6, Rmax=resources[i], warmstart_info=grid_variation[-1][0], warmstart_opts = 1e-3))
        #res_var_list.append(
...
vary_resources(steps = 46, ir_t=15, ir_s = 5, fr_s=15, fr_t=45, start=5)