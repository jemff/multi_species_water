#REMARK THIS VERSION GIVES TIGHTER MIGRATIONS THAN THE PUBLISHED VERSION
#REMARK NO VARIATION IN DVM TYPE HERE
#THIS IS DUE TO A DIFFERENT SET OF PARAMETERS
#THE RESULTS IN THE PUBLISHED VERSION CAN ALSO BE RECREATED HERE BY GOING TO MORE EXTREME PARAMETER RANGES THIS IS COMPUTATIONALLY VERY EXPENSIVE
...
#The best results were actually achieved by having the dumbest possible integrals here, i.e. removing Mx.M and dividing by the total number of points, and a tolerance of 10^-8. Having precise integrals probably introduces too tight a coupling for the numerical scheme.
    ...
    ubg = np.concatenate([np.zeros(probs), 1e-8*np.ones(2), 1e-8*np.ones(6), 1e-6*np.ones(2)])#, np.repeat(10**(-6), g.size()[0] - probs)])
