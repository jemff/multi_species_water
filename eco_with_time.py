...
Mx = discrete_patches(50, 20) #simple_method(50, 80)#spectral_method(50, 10, segments = 1) #spectral_method(30, 30, segments = 1)
...
for j in range(tot_times):
    ...
    beta_z.append(max(light_levels)/masses[0] * upright_wc**0+beta_0)
    beta_ff.append(beta_i[1] * upright_wc/(1+upright_wc)+beta_0)
    beta_lp.append(beta_i[2] * upright_wc/(1+upright_wc)+beta_0)
    beta_ld.append(beta_i[3] * upright_wc/(1+upright_wc)+beta_0)
...
for j in range(tot_times):
    ...
    ld_loss_b.append(com_ld * sigma_ld_b[j]**2)
#Bentic dynamics
...
for j in range(tot_times):
    ...
    lp_dyn.append(state[j][2]*(epsi[2]*(c_lp*state[j][1]*lp_ff_enc[j]*lp_satiation[j] - f_c) - lp_loss[j] - bg_M - lp_loss[j]))
    ld_dyn.append(state[j][3]*(epsi[3]*(c_ld*state[j][1]*ld_ff_enc[j]*ld_satiation[j] + state[j][4]*ld_bc_enc[j]*c_ld*ld_satiation[j] - f_c)- ld_loss[j] -ld_loss_b[j] - bg_M))
...
df_ld_b = []
for j in range(tot_times):
    df_z.append(epsi[0]*c_z**2*beta_z[j]*res_level[j]*z_satiation[j]**2 - 1/(epsi[0])*state[j][1]*c_ff*sigma_ff[j]*beta_ff[j]*ff_satiation[j] + lam[j][0]) #epsi[0]*
    ...
    df_ld_up.append(epsi[3]*state[j][1]*sigma_ld[j]*c_ld**2*beta_ld[j]*ld_satiation[j]**2 - com_ld* beta_ld[j]*sigma_ld[j] + lam[j][3])
    df_ld_b.append(epsi[3]*beta_ld_b[j]*0.5*state[j][-1]*c_ld**2*ld_satiation[j]**2 - com_ld * beta_ld_b[j] * sigma_ld_b[j] + lam[j][3])
...
#diff_eq_z, diff_eq_ff, diff_eq_lp, diff_eq_ld, diff_eq_bc,
g = ca.vertcat(*[*diff_eqs, probability, *complementarity, *normal_cone])
lbg = np.zeros(g.size()[0])#np.zeros(2*tot_times+5*tot_points*tot_times+4*tot_times + 5) #5*tot_times +
ubg = ca.vertcat(*[*np.zeros(g.size()[0]-(4*tot_points*tot_times + tot_times)), [ca.inf]*(4*tot_points*tot_times + tot_times)]) #+ 5*tot_times
