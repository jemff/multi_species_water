...
Mx = spectral_method(50, 50) #simple_method(50, 80)#spectral_method(50, 10, segments = 1) #spectral_method(30, 30, segments = 1)
light_levels = [0, 0.2, 1] #[0, 20, 40, 20]
time_lengths = [0.4, 0.2, 0.4] #[0.4, 0.1, 0.4, 0.1] #sum to 1
...
for j in range(tot_times):
    ...
    beta_ff.append(beta_i[0] * upright_wc)
    beta_lp.append(beta_i[1] * upright_wc)
    beta_ld.append(beta_i[2] * upright_wc)
...
for j in range(tot_times):
    ...
    ld_satiation.append(1/(state[j][0]*ld_ff_enc[j]+state[j][-1]*ld_bc_enc[j]+c_ld))
...
for j in range(tot_times):
    res_dyn.append(r * (rz - res_level[j]) - state[j][0] * c_ff * beta_ff[j] * res_level[j] * sigma_ff[j] * ff_satiation[j])
    bent_dyn.append(r_b * (Bmax - state[j][3]) - c_ld*state[j][2]*state[j][2]*ld_bc_enc[j] * ld_satiation[j])
    ff_dyn.append(state[j][0]*(epsi[0]*(c_ff*ff_z_enc[j]*ff_satiation[j] - f_c)- state[j][1] * c_lp * lp_ff_enc[j] * lp_satiation[j] - state[j][2] * ld_ff_enc[j] * ld_satiation[j] - bg_M))
    lp_dyn.append(state[j][1]*(epsi[1]*(c_lp*state[j][0]*lp_ff_enc[j]*lp_satiation[j] - f_c) - lp_loss - bg_M))
    ld_dyn.append(state[j][2]*(epsi[2]*(c_ld*state[j][0]*ld_ff_enc[j]*ld_satiation[j] + state[j][3]*ld_bc_enc[j]*c_ld*ld_satiation[j] - f_c)- ld_loss - bg_M))
...
for j in range(tot_times):
    ...
    df_lp.append((epsi[1])*state[j][0]*sigma_lp[j]*c_lp**2*beta_lp[j]*lp_satiation[j]**2 + lam[j][1]) #-loss derivative
    df_ld_up.append(epsi[2]*state[j][0]*sigma_ld[j]*c_ld**2*beta_ld[j]*ld_satiation[j]**2 + lam[j][2])
    df_ld_b.append((epsi[2])*beta_ld_b[j]*0.5*state[j][3]*c_ld**2*ld_satiation[j]**2 + lam[j][2])

diff_eqs = []
#res_level_dyn = []
for j in range(tot_times):
    diff_eqs.append((state[j][0] - state[j-1][0] - time_lengths[j-1]*ff_dyn[j-1]))
    diff_eqs.append((state[j][1] - state[j-1][1] - time_lengths[j-1]*lp_dyn[j-1]))
    diff_eqs.append((state[j][2] - state[j-1][2] - time_lengths[j-1]*ld_dyn[j-1]))
    diff_eqs.append((state[j][3] - state[j-1][3] - time_lengths[j-1]*bent_dyn[j-1]))
...
ubg = ca.vertcat(*[*np.zeros(g.size()[0]-(3*tot_points*tot_times + tot_times)), [ca.inf]*(3*tot_points*tot_times + tot_times)])
