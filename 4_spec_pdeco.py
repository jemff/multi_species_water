...
def output(tot_points = 20, fidelity = 20, Rmax = 5, Bmax = 0.1, warmstart_info = None, warmstart_opts = 1e-3):
    ...
    if warmstart_info is None:
        ...
        s_opts = {'ipopt': {'print_level': 3, 'linear_solver': linsol, 'max_iter':35000, 'tol': 1e-5, #-5 works well with -3 below
                              ...
                              'warm_start_slack_bound_push': warmstart_opts, 'warm_start_mult_bound_push': warmstart_opts, 'limited_memory_max_history': 20, 'limited_memory_initialization':'scalar2'}} #Remark that -3 works well, esp. with -5 above.
...
for k in range(10):
    for j in range(10):
        grid_variation.append(increase_resolution(ir=15, fr=60, jumpsize=15, Bmax = 0.1*(k+1), Rmax = 5*(j+1)))
